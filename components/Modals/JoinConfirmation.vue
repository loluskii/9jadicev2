        ...
        <div class="row my-2 text-white">
          <div class="col-12 text-center">
            ...
            <div id="confirmationFeedback">
              <span v-if="loading" class="alert alert-primary py-1"
                ><span>{{ loadingMsg }}</span></span
              >
              <span v-if="error" class="alert alert-danger py-1"
                ><span>{{ errorMsg }}</span></span
              >
            </div>
            <div
...
export default {
  ...
  methods: {
    ...
    closeModal(id) {
      (this.error = false), this.$bvModal.hide(`joinConfirmation_${id}`);
    },
  },
};
</script>
